...
行级数据权限：普通用户强制机构过滤（org 模式覆盖条件、org 模式外归属校验），
未分配机构拒绝（deny），admin/认证未启用不限制。
"""
...
def _org_access(request: Request) -> OrgAccess:
    return OrgAccess.for_user(getattr(request.state, "user", None))


def _org_scoped_cond(request: Request, cond: dict) -> tuple[dict, JSONResponse | None]:
    """列表/导出查询的机构强制：deny → 403；org → 无条件覆盖 orgCode（忽略客户端伪造）。"""
    access = _org_access(request)
    if access.mode == "deny":
        return cond, JSONResponse(status_code=403, content={
...
def _org_scoped_detail(request: Request, owner: dict[str, Any] | None) -> JSONResponse | None:
    """详情接口的机构校验：org 模式下记录不属于本机构一律 404（不泄露存在性）。"""
    access = _org_access(request)
    if access.mode == "deny":
        return JSONResponse(status_code=403, content={
            "error": "当前账号未分配机构，无法查询业务数据，请联系管理员分配"})
    if access.mode == "org" and owner and owner.get("org_code") != access.org_code:
...
async def contract_query(request: Request):
    ...
    cond, denied = _org_scoped_cond(request, body.get("conditions") or {})
    if denied:
        return denied
    result = await _svc(request).query_contract(cond, page, size, sort_by, sort_order)
...
async def preserve_query(request: Request):
    ...
    if denied:
        return denied
    result = await _svc(request).query_preserve(cond, page, size, sort_by, sort_order)
    return JSONResponse(content=result)
...
async def claim_query(request: Request):
    ...
    if denied:
        return denied
    result = await _svc(request).query_claim(cond, page, size, sort_by, sort_order)
    return JSONResponse(content=result)
...
async def contract_detail(policy_id: str, request: Request):
    ...
    denied = _org_scoped_detail(request, detail.get("policy"))
    if denied:
        return denied
...
async def preserve_detail(preserve_id: str, request: Request):
    ...
    denied = _org_scoped_detail(request, detail.get("preserve"))
    if denied:
        return denied
    return JSONResponse(content=detail)
...
async def claim_detail(claim_id: str, request: Request):
    ...
    denied = _org_scoped_detail(request, detail.get("claim"))
    if denied:
        return denied
    return JSONResponse(content=detail)
...
async def contract_export(request: Request):
    ...
    if denied:
        return denied
    csv_text = await _svc(request).export_contract(cond)
    return Response(
        content=csv_text,

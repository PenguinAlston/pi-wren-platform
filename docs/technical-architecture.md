...
| 模块 | 职责 | 关键实现 |
|---|---|---|
| `apps/web` | 聊天控制台 + 管理页 | Next.js 15、**SSE 实时轨迹**、多 Agent 切换、结果表、`/agents` 自定义 Agent 管理页（MDL/连接测试/启停/编辑/池监控） |
...
| `services/agent-registry` | **自定义 Agent 注册表** | `sys_agent_config` 持久化、AES-256-GCM 连接串加密、运行时动态加载/更新/注销、错误隔离、审计、连接池监控 |
...
| `infra/postgres` | 数据初始化 | 建表 + 种子（保险 22 张生产级表 + 财务 demo + `sys_agent_config` + `sys_operation_log` 审计 + UADMIN） |
